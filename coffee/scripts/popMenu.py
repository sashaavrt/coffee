...
def popMenu(start_date, end_date):
    df = pd.read_csv(url)
    ...
    sns.barplot(x='sales', y='category', hue='category', data=category_aov.sort_values('sales', ascending=False))
    ...
    category_aov.to_csv(buffer_table, index=False)
    buffer_table.seek(0)
